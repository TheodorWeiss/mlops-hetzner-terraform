...
def get_current_champion_mae(conn) -> float:
    sql = """
        ...
        WHERE model_type = 'lightgbm_delta_time_geo_weather'
          AND promotion_decision IN (
              'selected_as_weather_required_champion',
    ...
    if not row:
        raise RuntimeError("No LightGBM weather champion MAE found.")
...
def main():
    ...
    with get_connection() as conn:
        ...
        run_name = f"xgboost_delta_time_geo_weather_challenger_test_{window['test_month']}"
        model_type = "xgboost_delta_time_geo_weather_challenger"
        ...
        client.set_model_version_tag(
            ...
            version=version.version,
            key="role",
            value="serving_delta_weather_challenger",
        )
        client.set_model_version_tag(
            name=REGISTERED_MODEL_NAME,
            version=version.version,
        ...
        result = {
            ...
            "notes": (
                f"XGBoost delta weather challenger. champion_mae={champion_mae:.6f}. "
    ...
    print("XGBOOST_DELTA_WEATHER_CHALLENGER_OK")
