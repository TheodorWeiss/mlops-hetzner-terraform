...
def evaluate(conn, target_name: str, prediction_column: str, model_type: str):
    sql = f"""
        WITH eval_data AS (
            SELECT
    ...
    run_name = f"{model_type}_{target_name}_202604_fair"
    notes = "Fair baseline on the same feature-ready rows as LightGBM."
...
def main():
    baselines = [
        ("departures", "lag_24h_departures", "baseline_lag_24h_fair"),
        ("departures", "lag_168h_departures", "baseline_lag_168h_fair"),
        ("returns", "lag_24h_returns", "baseline_lag_24h_fair"),
        ("returns", "lag_168h_returns", "baseline_lag_168h_fair"),
    ]

    with get_connection() as conn:
        for target_name, prediction_column, model_type in baselines:
            run_id, rows_test, mae, rmse, mape = evaluate(
                conn,
                target_name,
                prediction_column,
                model_type,
